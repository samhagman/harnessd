...
### Planner mode
Read-only agent that expands a user objective into `SPEC.md`, `packets.json`, `risk-register.json`, and `plan-summary.md`. Supports web research via perplexity, interactive interviews (`--context`), and prior-run context.
...
cd harness && npx tsx src/main.ts --context "your objective"
