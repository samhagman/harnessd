...
export async function negotiateContract(
  ...
  for (let round = 1; round <= maxRounds; round++) {
    ...
    const contractBuilderCapabilities = {
    ...
    const effectiveContractBuilderMcp = Object.keys(contractBuilderMcpServers).length > 0
      ? contractBuilderMcpServers
      : {
    ...
    const builderResult = await runWorker(
      contractBuilderBackend,
      {
        ...
        mcpServers: effectiveContractBuilderMcp,
        ...(contractBuilderBackend.supportsOutputSchema() ? {
          outputSchemaPath: path.join(_schemasDir, "contract-proposal.json"),
        } : {}),
    ...
    const evaluatorPrompt = buildContractEvaluatorPrompt(proposal, riskRegister, negConfig.config.enableMemory, factory.forRole("contract_evaluator").supportsMcpServers());
    ...
    const effectiveContractEvaluatorMcp = Object.keys(contractEvaluatorMcpServers).length > 0
      ? contractEvaluatorMcpServers
      : {
          "harnessd-validation": createValidationMcpServer(),
          ...(negConfig.memory ? { "harnessd-memory": createMemorySearchMcpServer(negConfig.memory) } : {}),
          ...createResearchMcpServerRecord(negConfig.config.researchTools),
    ...
    const evaluatorResult = await runWorker(
      contractEvaluatorBackend,
      {
        ...
        mcpServers: effectiveContractEvaluatorMcp,
        ...(contractEvaluatorBackend.supportsOutputSchema() ? {
          outputSchemaPath: path.join(_schemasDir, "contract-review.json"),
        } : {}),
        sandboxMode: "read-only",
      },
      {
