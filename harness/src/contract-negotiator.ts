...
export async function negotiateContract(
  ...
  for (let round = 1; round <= maxRounds; round++) {
    ...
    const builderBackendCapabilities = {
      supportsMcpServers: contractBuilderBackend.supportsMcpServers(),
      nudgeStrategy: contractBuilderBackend.nudgeStrategy(),
      supportsOutputSchema: contractBuilderBackend.supportsOutputSchema(),
    };
    const builderPrompt = buildContractBuilderPrompt(
      ...
      builderBackendCapabilities,
    );

    const contractBuilderBuffer = negConfig.memory ? new MemvidBuffer(negConfig.memory) : null;

    const contractBuilderMcpServers = buildRoleMcpServers(contractBuilderBackend, {
      memory: negConfig.memory,
      researchTools: negConfig.config.researchTools,
    });
    ...
    const builderResult = await runWorker(
      contractBuilderBackend,
      {
        ...
        mcpServers: contractBuilderMcpServers,
        ...(contractBuilderBackend.supportsOutputSchema() ? {
          outputSchemaPath: path.join(SCHEMAS_DIR, "contract-proposal.json"),
        } : {}),
    ...
    const evaluatorPrompt = buildContractEvaluatorPrompt(
      proposal,
      riskRegister,
      negConfig.config.enableMemory,
      contractEvaluatorBackend.supportsMcpServers(),
    );

    const contractEvaluatorBuffer = negConfig.memory ? new MemvidBuffer(negConfig.memory) : null;

    const contractEvaluatorMcpServers = buildRoleMcpServers(contractEvaluatorBackend, {
      memory: negConfig.memory,
      researchTools: negConfig.config.researchTools,
    });
    ...
    const evaluatorResult = await runWorker(
      contractEvaluatorBackend,
      {
        ...
        mcpServers: contractEvaluatorMcpServers,
        ...(contractEvaluatorBackend.supportsOutputSchema() ? {
          outputSchemaPath: path.join(SCHEMAS_DIR, "contract-review.json"),
        } : {}),
        sandboxMode: "read-only",
      },
      {
