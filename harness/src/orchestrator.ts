...
import { DevServerManager } from "./background-jobs.js";
...
async function handlePacketSelection(
  ...
  const isRound2 = runState.round === 2;
...
async function executeBuilder(
  ...
  const completionSummaries = readCompletionSummaries(repoRoot, runState.runId, runState.completedPacketIds);
  return runBuilder(factory.forRole("builder"), contract, {
    ...
    packetId,
    config,
  }, readSpec(repoRoot, runState.runId), readRiskRegister(repoRoot, runState.runId), evalReport, contextOverrides, completionSummaries);
}
...
async function handleBuilding(
  ...
  const result = await executeBuilder(factory, repoRoot, workspaceDir, runState, config);
...
async function handleEvaluation(
  ...
  const result = await runEvaluator(factory.forRole("evaluator"), contract, builderReport, {
    ...
    packetId,
    config,
  }, readRiskRegister(repoRoot, runState.runId), evaluatorGuide, completionSummaries, gateResultsSummary);
...
async function handleFixing(
  ...
  const result = await executeBuilder(factory, repoRoot, workspaceDir, runState, config, evalReport);
...
async function handleQAReview(
  ...
  let devServerMgr: DevServerManager | undefined;
  if (config.devServer) {
    try {
      devServerMgr = new DevServerManager(config.devServer, workspaceDir);
      const handle = await devServerMgr.ensureRunning();
      devServerUrl = handle.url;
    } catch (err) {
      console.log(`[${runState.runId}] Dev server failed to start: ${errorStr(err)}. QA will proceed without it.`);
    }
  }
  ...
  try {
    ...
    if (devServerMgr) {
      try { await devServerMgr.stop(); } catch {}
    }
  }
}
...
async function handleRound2Planning(
  ...
  // Load QA report from current round
  const qaReport = readQAReport(repoRoot, runState.runId, runState.round);
  if (!qaReport) {
    console.error(`[${runState.runId}] Cannot start R2 planning: no QA report found for round ${runState.round}`);
    return transition(repoRoot, runState, "needs_human", "No QA report available for R2 planning");
  }
  ...
  if (!result.success) {
    console.log(`[${runState.runId}] Round 2 planner failed: ${result.error}`);
    return null; // retry
  }
  ...
  appendEvent(repoRoot, runState.runId, {
    ...
    phase: "round2_planning",
    detail: `${result.packets.length} fix packets planned`,
  });

  const round2PacketOrder = result.packets.map((p) => p.id);

  // Transition to R2 approval gate
  appendEvent(repoRoot, runState.runId, {
...
export function selectNextPacket(
  ...
  // Use round 2 packet order if in round 2 and round2PacketOrder is populated
...
function readRound2Packets(repoRoot: string, runId: string): Packet[] {
  try {
    return readArtifact(repoRoot, runId, "spec/packets-r2.json", z.array(PacketSchema));
  } catch {
    return [];
  }
}
...
function readQAReport(repoRoot: string, runId: string, round: number): QAReport | undefined {
  try {
    return readArtifact(repoRoot, runId, `spec/qa-report-r${round}.json`, QAReportSchema);
  } catch {
    return undefined;
  }
}
...
function gatherAllContracts(repoRoot: string, runId: string, runState: RunState): PacketContract[] {
  ...
  for (const packetId of runState.completedPacketIds) {
    try {
...
function writeCompletionSummary(
  ...
  try {
    const packets = readPackets(repoRoot, runId);
    const packet = packets.find((p) => p.id === packetId);
    if (!packet) return;
