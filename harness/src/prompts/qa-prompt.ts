...
${ctx.evaluatorGuide.edgeCases.map((e) => `- ${e}`).join("\n")}`);
  ...
  const devServerInfo = ctx.devServerUrl
    ? `The dev server is already running at: ${ctx.devServerUrl}`
    : ctx.devServerCommand
      ? `Start the dev server with: \`${ctx.devServerCommand}\``
      : `Start the dev server with: \`npm run dev\` (or the project-appropriate command)`;
...
### Phase 1: App Startup
1. Ensure the dev server is running and the app loads
...
schema_name="QAReport" and your JSON to check it's valid. Fix any errors before emitting.`);
