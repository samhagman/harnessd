...
export async function runEvaluator(
  ...
  // Fallback to legacy in-process construction when MCP is not supported (FakeBackend)
  const effectiveMcpServers = Object.keys(mcpServers).length > 0
    ? mcpServers
    : {
        "harnessd-validation": createValidationMcpServer(criterionIds),
  ...
  const workerResult = await runWorker(
    backend,
    {
      ...
      ...(backend.supportsOutputSchema() ? {
        outputSchemaPath: path.join(_schemasDir, "evaluator-report.json"),
      } : {}),
