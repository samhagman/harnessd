...
export async function runPlanReview(
  ...
  const planReviewMcpServers = buildRoleMcpServers(backend, {
    memory: opts.memory,
    researchTools: opts.config.researchTools,
  });
  const effectivePlanReviewMcp = Object.keys(planReviewMcpServers).length > 0
    ? planReviewMcpServers
    : {
  ...
  const workerResult: WorkerResult<PlanReview> = await runWorker(
    backend,
    {
      ...
      mcpServers: effectivePlanReviewMcp,
      ...(backend.supportsOutputSchema() ? {
        outputSchemaPath: path.join(_schemasDir, "plan-review.json"),
      } : {}),
      hooks: {
        PreToolUse: [
