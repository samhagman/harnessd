...
// MCP server creation (used for Claude backends only; Codex uses buildRoleMcpServers)
...
export async function runBuilder(
  ...
  // Fallback to legacy in-process MCP construction when buildRoleMcpServers
  // returns an empty record (FakeBackend in tests) — keep existing Claude-only
  // mcpServers block so tests that assert on the runWorker call don't break.
  const effectiveMcpServers = Object.keys(mcpServers).length > 0
    ? mcpServers
    : {
  ...
  const workerResult = await runWorker(
    backend,
    {
      ...
      ...(backend.supportsOutputSchema() ? {
        outputSchemaPath: path.join(_schemasDir, "builder-report.json"),
      } : {}),
